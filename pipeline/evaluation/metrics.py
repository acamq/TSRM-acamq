...
def compute_metrics_per_group(
    ...
    """Aggregate per (block, satellite) then macro-average.

    This ensures statistical independence by treating each (block, sat)
    as an independent sample unit.

    Args:
        imputed_list: list of imputed arrays, one per (block, sat)
        truth_list: list of ground truth arrays
        mask_list: list of boolean masks for artificially missing positions
        group_ids: list of (block_id, sat_id) tuples

    Returns:
        dict with mse, mse_std, mae, mae_std, per_group details, n_groups
    """
    group_metrics = []

    for (block_id, sat_id), imp, tru, msk in zip(group_ids, imputed_list, truth_list, mask_list):
        flat_imp = imp.reshape(-1)
        flat_tru = tru.reshape(-1)
        flat_msk = msk.reshape(-1)

        masked_imp = flat_imp[flat_msk]
        masked_tru = flat_tru[flat_msk]
        ...
        group_metrics.append({
            'block': block_id,
            'sat': sat_id,
            'mse': float(np.mean((masked_imp - masked_tru) ** 2)),
            'mae': float(np.mean(np.abs(masked_imp - masked_tru))),
            'n': len(masked_imp)
        })

    if not group_metrics:
        return {
            'mse': None, 'mse_std': None,
            'mae': None, 'mae_std': None,
            'per_group': [], 'n_groups': 0
        }

    mses = [g['mse'] for g in group_metrics]
    maes = [g['mae'] for g in group_metrics]
